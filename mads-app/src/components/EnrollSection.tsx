...
function formatTelegram(value: string): string {
  const nick = value.replace(/^@+/, '').replace(/[^a-zA-Z0-9_]/g, '').slice(0, 32);
  if (!nick) return '';
  return `@${nick}`;
}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 space-y-6">
            <h2 className="font-unbounded text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight text-[#121214]">
              Заявка как в боевой версии
            </h2>
            <p className="text-base text-[#52525B] leading-relaxed">
              Форма учебная: данные никуда не уходят, оплата не списывается. В реальном запуске здесь был бы Telegram или CRM.
            </p>
              ...
              {isSubmitted ? (
                <div className="text-center py-12 space-y-4">
                  ...
                  <h3 className="font-unbounded text-2xl sm:text-3xl font-bold text-[#121214]">
                    На учебном сайте заявка никуда не уходит.
                  </h3>
                  <p className="text-sm text-[#52525B] max-w-md mx-auto leading-relaxed">
                    {formData.firstName}, это проверка интерфейса. В боевой версии здесь было бы письмо или сообщение в мессенджер.
                  </p>
                  <div className="pt-6">
                    <button
                      onClick={() => {
